...
export class FinishScheulePage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public statusBar: StatusBar
    ...
    if (this.body['allday']) {
      this.start = this.datePipe.transform(new Date(new Date(this.body['start'].replace(/-/g, "/")).getTime() + 2 * 8 * 60 * 60 * 1000), 'yyyy-MM-dd')
      this.stop = this.datePipe.transform(new Date(new Date(this.body['stop'].replace(/-/g, "/")).getTime() + 2 * 8 * 60 * 60 * 1000), 'yyyy-MM-dd')
    }
    else {
    ...
    // this.relly_stop_time = this.datePipe.transform(new Date(), 'yyyy-MM-dd HH:mm').replace(' ','T')+'Z';
    var relly_start = this.datePipe.transform(new Date(new Date(this.body['relly_start_time_temp'].replace(/-/g, "/")).getTime() + 8 * 60 * 60 * 1000), 'yyyy-MM-dd HH:mm')
    this.relly_start_time = this.datePipe.transform(new Date(relly_start.replace(/-/g, "/")), 'yyyy-MM-dd HH:mm');
    this.relly_stop_time = this.datePipe.transform(new Date(), 'yyyy-MM-dd HH:mm');
  ...
  finish() {
    ...
    this.body['rt_task_progress'] = 100
    ...
    this.firService.write_wait_thing_new({
      'body': this.body, 'data': {
