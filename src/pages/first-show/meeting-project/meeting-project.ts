...
/**
 * Generated class for the MeetingProjectPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-meeting-project',
  templateUrl: 'meeting-project.html',
  providers: [FirstShowService, DatePipe]
})
export class MeetingProjectPage {
  ...
  ionViewDidEnter() {
    ...
    this.storage.get('user').then(res => {
      ...
      if (this.isEdit == true) {
        if (this.navParams.get('parent_project_id')) {
          this.parent_project_id = this.navParams.get('parent_project_id')
        }
        let current_day = new Date()
        current_day = this.navParams.get('date')
        if (this.navParams.get('create_new')) {
          this.navParams.data.create_new = false
